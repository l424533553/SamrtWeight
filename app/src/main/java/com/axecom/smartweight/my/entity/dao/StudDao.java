...
public class StudDao<T> {
    // ORMLite提供的DAO类对象，第一个泛型是要操作的数据表映射成的实体类；第二个泛型是这个实体类中ID的数据类型
    private Dao<T, Integer> dao;

    public StudDao(Context context) {
        try {
            this.dao = OrmliteBaseHelper.getInstance(context.getApplicationContext()).getDao(Stud.class);
    ...
    private static StudDao baseDao;

    public static StudDao getInstance(Context context) {
        if (baseDao == null) {
            baseDao = new StudDao(context);
        }
        return baseDao;
    }
